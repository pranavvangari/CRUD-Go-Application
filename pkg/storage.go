...
func (r *EntityMemoryRepository) Add(entity *Entity) error {
	r.mutex.Lock() //acquires the mutex lock to ensure thread-safety before modifying the entities slice.
	defer r.mutex.Unlock()

	if entity.ID == "" {
		atomic.AddUint64(&r.lastID, 1)          // increments the last id by 1
		entity.ID = fmt.Sprintf("%d", r.lastID) //converts the r.lastID to a string in order to set the new id as the number
	}
...
func (r *EntityMemoryRepository) List(page, pageSize int) ([]*Entity, error) {
	...
	if end > len(r.entities) { //checking for last page
